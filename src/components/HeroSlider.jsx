...
function HeroSlider({ newsList }){
    ...
    const { id, title, date,  summary, image, news_source } = todaysNews[current];

    return (
        <>
            <div className="hero-slider" style={{ backgroundImage: `url(${image})`, transition: 'opacity 0.3s ease-in-out', cursor: 'pointer'}} onClick={() => navigate(`/main/${id}`)}>
